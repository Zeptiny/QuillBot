...
async def main():
    ...
    try:
        ...
        con = sqlite3.connect(tmpdb)
        con.execute("DROP TABLE chunks_fts_config")
        con.commit()
        con.close()
        repaired = HistoryRAG(bot=None)
        repaired._db_path = lambda: tmpdb  # type: ignore[method-assign]
        repaired._ensure_db()
        out = repaired._exec_sql_sync(GID, f"SELECT c.msg_id FROM chunks c JOIN chunks_fts f ON f.msg_id=c.msg_id WHERE f.guild_id={GID} AND chunks_fts MATCH 'lag' ORDER BY c.msg_id", 5.0, 50)
        check("fts health check rebuilds corrupted index", out.split("\n")[1:] == ["1", "2", "5"], out)
